...
{
    public partial class ExtrusionPackingManagementMainView : Form
    {
        //Fields
        string dataIn;
        double weightRT;
        ...
        public ExtrusionPackingManagementMainView()
        {
            InitializeComponent();
        }
        ...
        private void LoadData2DTGV(string key)
        {
            ...
            {
                sb.Append("select * from extrusion_realtime where permission_dept = 'Packing' and create_date > '" + dateStart + "' and create_date < '" + dateEnd + "' order by create_date desc");
            }
            else
            {
                sb.Append("select * from extrusion_realtime where permission_dept = 'Packing' and product_code like '%" + key.ToLower() + "%' order by create_date desc");
            }
            ...
            dtgvHistoryList.Columns["length"].HeaderText = "Chiều dài\r\n长度";
        ...
        {
            if (String.IsNullOrEmpty(cbxCodeList.Text.Trim()) || String.IsNullOrEmpty(txbLength.Texts) || String.IsNullOrEmpty(txbEmpCode.Texts))
            {
                return false;
            }
            else
        ...
        private void txbSearchCode__TextChanged(object sender, EventArgs e)
        {
            ...
            {
                LoadCodeList2ComboBox(String.Empty);
                txbEmpCode.Texts = String.Empty;
                txbLength.Texts = String.Empty;
                label1.Focus();
            }
        }

        private void ExtrusionPackingManagementMainView_FormClosing(object sender, FormClosingEventArgs e)
        {
                ...
                if (checkNull())
                {
                    ...
                    if (dialogResult == DialogResult.OK)
                    {
                        ...
                        if (!String.IsNullOrEmpty(reEmp))
                        {
                            ...
                            d.Length = Convert.ToDouble(txbLength.Texts.Trim());
                            d.Receiver = reEmp;
                            ...
                            new ThreadStart(() =>
                            {
                                ...
                                queryInsertData.Append("('" + UUIDGenerator.getAscId() + "', '" + d.MainCode + "', " + d.Length + ", " + d.Weight + ", N'" + d.Sender + "', N'" + d.Receiver + "', '" + d.Date + "', 'Packing')");
        ...
        private void btnSaveExcel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                queryGetData.Append("select * from extrusion_realtime where permission_dept = 'Packing' and create_date >= '" + dtpDateIn.Value.ToString("yyyy-MM-dd HH:mm:ss") + "' and create_date <= '" + dtpDateOut.Value.ToString("yyyy-MM-dd HH:mm:ss") + "'");
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    ExcelSave.SaveExcel_ExtrusionPacking(details);
                }
                else
                {
        ...
        private void ExtrusionPackingManagementMainView_Load(object sender, EventArgs e)
        {
